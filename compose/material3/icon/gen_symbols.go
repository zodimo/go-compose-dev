...
func main() {
	...
	fmt.Fprintln(out, "// SymbolName implements IconSource, so it can be used directly with Icon().")
	fmt.Fprintln(out, "type SymbolName string")
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "// isIconSource implements the IconSource interface.")
	fmt.Fprintln(out, "func (SymbolName) isIconSource() {}")
	fmt.Fprintln(out, "")
