...
func TestTextFieldState_UndoRedo(t *testing.T) {
	state := NewTextFieldState("")
	...
	if state.Text() != "" {
		t.Errorf("expected empty after undo, got '%s'", state.Text())
	}
	...
	if state.Text() != "Hello" {
		t.Errorf("expected 'Hello' after redo, got '%s'", state.Text())
	}
}

func TestTextFieldState_ClearHistory(t *testing.T) {
	state := NewTextFieldState("")
	...
	if state.CanUndo() {
		t.Error("expected CanUndo false after clear history")
	}
}
